...
const kv: KV = {
  async get(k) {
    if (REDIS_ENABLED && raw) return raw.get(k);
    return mem.get(k) ?? null;
  },
  async set(k, v) {
    if (REDIS_ENABLED && raw) { await raw.set(k, v); return; }
    mem.set(k, v);
  },
  async del(k) {
    if (REDIS_ENABLED && raw) return raw.del(k);
    const existed = mem.delete(k);
    if (timers.has(k)) { clearTimeout(timers.get(k)!); timers.delete(k); }
    return existed ? 1 : 0;
  },
  async expire(k, s) {
    if (REDIS_ENABLED && raw) { await raw.expire(k, s); return; }
...
export async function connectRedisIfEnabled() {
  if (!REDIS_ENABLED) {
    logger.info('[redis] Deshabilitado (se usa fallback in-memory)');
    return;
  }
  if (raw && raw.isOpen) {
    logger.info('[redis] Ya conectado');
    return;
  }
  ...
  raw.on('error', (err: any) => logger.error({ err }, '[redis] Error'));
  await raw.connect();
  logger.info({ url: process.env.REDIS_URL }, '[redis] Conectado a');
}
