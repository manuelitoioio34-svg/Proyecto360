  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        ...
        // Solid backgrounds for readability (no transparency)
        "bg-white dark:bg-slate-900 hover:bg-gray-50 dark:hover:bg-slate-800",
        // Sizing
        "inline-flex w-full items-center justify-between gap-2 rounded-md border px-3 py-2 text-sm whitespace-nowrap shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50",
        // Value styles
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          // Solid background + text colors for visibility
          "bg-white text-slate-900 dark:bg-slate-900 dark:text-slate-100",
          // Animations and surface
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "text-slate-900 dark:text-slate-100 focus:bg-accent focus:text-accent-foreground [&_svg:not([class*='text-'])]:text-muted-foreground relative flex w-full cursor-default items-center gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm outline-hidden select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        // Center item contents and span (ItemText) so option content appears centered
        "justify-center text-center pl-10 pr-10 *:[span]:last:w-full *:[span]:last:justify-center *:[span]:last:text-center",
        className
      )}
      {...props}
    >
